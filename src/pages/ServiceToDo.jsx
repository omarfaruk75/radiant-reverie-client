    ...
    useEffect(() => {
        const getData = async () => {
            try {
                ...
                console.log(data);
                // Filter out services belonging to the current user
                const filteredServices = data.filter(service => service.provider.email !== user?.email);
                if (!filteredServices) return toast.error('There is no Booked Job')
